  ...
  while ((match = insertRegex.exec(sql)) !== null) {
    ...
    values.forEach((value) => {
      ...
      if (kode.length === 2) {
        ...
        // Kecamatan
        const kabupatenId = cleanKode.slice(0, 4); // Ambil 4 digit pertama untuk kabupaten_id
        const kecamatanCode = `${cleanKode.slice(0, 2)}.${cleanKode.slice(2, 4)}.${cleanKode.slice(4, 6)}`; // Format kode titik
        kecamatan.push({
          id: cleanKode,
        ...
        // Kelurahan
        const kecamatanId = cleanKode.slice(0, 6); // Ambil 6 digit pertama untuk kecamatan_id
        const kelurahanCode = `${cleanKode.slice(0, 2)}.${cleanKode.slice(2, 4)}.${cleanKode.slice(4, 6)}.${cleanKode.slice(6, 10)}`; // Format kode titik
        kelurahan.push({
          id: cleanKode,
